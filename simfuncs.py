...
# updates states of individuals in proper matrix
def gens(mat, mat3):
...
# updating states based on incubation and recovery matricies
...
# checks whether an individual will become infected in a stochastic manner where the more people
# surrounding them means more probability of infection
def infected(count):
    ran = random.random()
    if count == 0:
...
# checks whether an individual has recovered
...
# stochastically determines the recovery time of an individual
def recoverydesig():
    rand = random.random()
...
# simulates the passage of time where the recovery and incubation matricies are incremented
def time(mat3, mat4):
    for y in range(len(mat3)):
        for x in range(len(mat3[y])):
            if y > 0 and y < simvars.length - 1 and x > 0 and x < simvars.length - 1:
...
# The incubation time for an infected individual is stochastically determined
def incub():
    rand = random.random()
...
# this sets up the initial matrix of health status based on the preset probability of starting infected
...
# this sets the specificed matrix to have the place holder position of posplace specified in the simvars file
def prefillcounter(mat):
    for y in range(len(mat)):
        for i in range(simvars.length):
            mat[y].append(simvars.posplace)


# setup of the matrix to have zeros around its edges to avoid indexing off the matrix
...
# infection statistics calcultaed for a given generation
def stats(mat):
    totalinfec = 0
    ...
    print("After " + str(simvars.gencount) + " generations, " + str(totalinfec) + " people out of a total of " + str(
        totalpop) + " have been infected.")
    print("That is a total infection rate of over " + str(avginfecpct) + "%.")


# creation matplotlib showing how infections have changed over the generations
def graphs(mat):
    popinfec = 0
    for y in range(len(mat)):
        for x in range(len(mat[y])):
